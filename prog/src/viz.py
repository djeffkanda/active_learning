...
def plot_compare_to_random_metrics(query_manager: TrainTestManager, random_manager: TrainTestManager, save_path):
    ...
    ax2.set_ylabel('Accuracy')
    ax2.legend()
    f.savefig(save_path + 'fig_pair.png')
    plt.show()
...
def plot_all_metrics(random_manager: TrainTestManager, entropy_manager: TrainTestManager,
    ...
    Args:
        ...
        lc_manager(TrainTestManager): TrainTestManager objects used with Least confidence query strategy
    ...
    # loss plot
    ax1.plot(num_data, random_manager.metric_values['global_test_loss'], '-o', label='Random querying')
    ax1.plot(num_data, entropy_manager.metric_values['global_test_loss'], '-o', label='Entropy querying')
    ax1.plot(num_data, lc_manager.metric_values['global_test_loss'], '-o',
             label='Least Conf. querying')
    ax1.plot(num_data, margin_manager.metric_values['global_test_loss'], '-o', label='Margin querying')
    ax1.set_title('Test Loss')
    ax1.set_xlabel('Number of data used')
    ax1.set_ylabel('Loss')
    ax1.legend()
    ...
    # accuracy plot
    ax2.plot(num_data, random_manager.metric_values['global_test_accuracy'], '-o', label='Random querying')
    ax2.plot(num_data, entropy_manager.metric_values['global_test_accuracy'], '-o', label='Entropy querying')
    ax2.plot(num_data, lc_manager.metric_values['global_test_accuracy'], '-o',
             label='Least Confidence querying')
    ax2.plot(num_data, margin_manager.metric_values['global_test_accuracy'], '-o', label='Margin querying')
    ax2.set_title('Test accuracy')
    ax2.set_xlabel('Number of data used')
    ax2.set_ylabel('Accuracy')
    ax2.legend()
    f.savefig(save_path + 'fig_all.png')
    plt.show()
